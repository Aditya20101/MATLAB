...
nRx = [1 2];
Eb_N0_dB = [0:60]; % multiple Eb/N0 values
for jj = 1:length(nRx)
    for ii = 1:length(Eb_N0_dB)
        ...
        yHat = sum(yHat,1); % adding values from all the receive chains% receiver - hard decision decoding
        ipHat = real(yHat)>0;
        % counting the errors
...
close all
figure
semilogy(Eb_N0_dB,theoryBer_nRx1,'b','LineWidth',2);
hold on
semilogy(Eb_N0_dB,theoryBer_nRx2,'r','LineWidth',2);
% axis([0 35 10^-5 0.5])
% grid on
